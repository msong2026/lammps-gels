...
def main(epsilon: float = 5, gamma0: float = 5, number_density: float = 0.2, r0: float = 1.2, 
         rho: float = 33, dratio: float = 0.5, num_particles: int = 1000):
    """Main function
    ...
    :param r0: Minimum of Morse potential.
    :param rho: Morse interaction alpha.
    ...
    # Specify the interaction coefficients
    # Morse: d0 alpha r0 cutoff
    with open("coeffs.in", "w") as coefffile:
        for type_1 in range(num_types):
            diam_i = diameters[type_1]
            coefffile.write(f"group type{type_1+1} type {type_1+1}\n")
            coefffile.write(f"variable d{type_1+1} equal {diam_i}\n")
            for type_2 in range(type_1, num_types):
                diam_j = diameters[type_2]
                mixed_diam = 0.5 * (diam_i + diam_j)
                d0 = epsilon + (2-diam_i-diam_j)*250
                coefffile.write(f"pair_coeff {type_1 + 1:d} {type_2 + 1:d}{keyword:s}{d0:g} {rho:g} "
                                  f"{r0*mixed_diam:g} {r_cut_coeff * mixed_diam:g}\n")
                # Specify the interaction coefficients for dpd/tstat
...
if __name__ == '__main__':
    ...
    if len(args) != 8:
        raise SyntaxError("Incorrect syntax.\nUse python gel_setup.py epsilon gamma0 number_density "
                          "r0 rho_0 dratio num_particles")
    epsilon = float(sys.argv[1])
    gamma0 = float(sys.argv[2])
    number_density = float(sys.argv[3])
    r0 = float(sys.argv[4])
    rho_0 = float(sys.argv[5])
    dratio = float(sys.argv[6])
    num_particles = int(sys.argv[7])
    main(epsilon, gamma0, number_density, r0, rho_0, dratio, num_particles)